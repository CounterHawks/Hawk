...
bot.on("ready", () => {
    console.log("Logged in as HawkBot#0109")
...
bot.on("message", message => {
    ...
    if (command === "ping") {
        message.channel.send(`UwU **(${Date.now() - message.createdTimestamp}ms)**`)
    }
    ...
    if (command === "kick") {
        ...
        if (!member.kickable)
            ...
            return message.channel.send("I simp for this user, dont kick him :(").then(msg => {
        msg.delete({ timeout: 30000 })
    })
        const reason = args.slice(1).join(" ")
        if (member) {
            if (!reason) return member.kick().then(member => {
                message.channel.send(`${member.user.tag} was kicked for no reason wtf? :/`);
            })

            if (reason) return member.kick().then(member => {
                message.channel.send(`${member.user.tag} was kicke ${reason}`);
            })
        }
    }
   ...
   message.channel.send("Messages has been Purged!").then(msg => {
        msg.delete({ timeout: 30000 })
    })
   }
